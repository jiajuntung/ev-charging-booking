    ...
    <div class="w-full max-w-md rounded-[32px] bg-white shadow-[0_18px_40px_rgba(0,0,0,0.10)] overflow-hidden border border-gray-100">
        ...
        <div class="px-8 py-10">
            ...
            <div class="text-center mb-8">
                ...
                <h1 class="text-3xl font-black text-[#664c35] tracking-tight">Welcome Back</h1>
                <p class="text-sm text-[#89a57b] mt-1 font-medium">Sign in to your account</p>
                <div class="w-10 h-1 bg-[#89a57b] mx-auto mt-3 rounded-full"></div>
            </div>

            <!-- Session Status -->
            @if (session('status'))
                <div class="mb-4 text-sm text-[#4a7a3d] bg-[#eaf3e6] border border-[#89a57b] rounded-xl px-4 py-3">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div class="flex items-center justify-between">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="remember"
                            class="w-4 h-4 rounded border-gray-300 accent-[#89a57b]">
                        <span class="text-sm text-gray-500">Remember me</span>
                    </label>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-sm text-[#89a57b] hover:text-[#664c35] font-medium transition">
                            Forgot password?
                        </a>
                    @endif
                </div>
                ...
                <p class="text-center text-sm text-gray-500">
                    ...
                    <a href="{{ route('register') }}" class="text-[#89a57b] hover:text-[#664c35] font-bold transition">Sign Up</a>
                </p>
